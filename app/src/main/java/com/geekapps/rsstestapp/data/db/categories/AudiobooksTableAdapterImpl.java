...
public class AudiobooksTableAdapterImpl extends BaseCategoryTableAdapter {
    ...
    public void addMediaItem(MediaItem media) {
        ...
        db.insert(dbHelper.TABLE_AUDIOBOOKS, null, getContentValues(media));
        db.close();
    }
    ...
    public void addMediaItems(List<MediaItem> medias) {
        ...
        for (int i = 0; i < medias.size(); i++)
            db.insert(dbHelper.TABLE_AUDIOBOOKS, null, getContentValues(medias.get(i)));
        db.close();
    }
    ...
    public MediaItem getMedia(int id) {
        ...
        Cursor cursor = db.query(dbHelper.TABLE_AUDIOBOOKS, new String[]{dbHelper.FIELD_ID, dbHelper.FIELD_NAME,
                        dbHelper.FIELD_ARTIST, dbHelper.FIELD_LOGO, dbHelper.FIELD_IS_FAVOURITE,
                        dbHelper.FIELD_POSITION}, dbHelper.FIELD_ID + "=?",
    ...
    public List<MediaItem> getAllMedias() {
        return getSomeMedias(dbHelper.TABLE_AUDIOBOOKS, null);
    }

    @Override
    public List<MediaItem> getFavouriteMedias() {
        return getSomeMedias(dbHelper.TABLE_AUDIOBOOKS, dbHelper.FIELD_IS_FAVOURITE + "=1");
    }

    @Override
    public long getMediasCount() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        long count = DatabaseUtils.queryNumEntries(db, dbHelper.TABLE_AUDIOBOOKS);
        db.close();
        return count;
    }
    ...
    public int updateMedia(MediaItem media) {
        ...
        return db.update(dbHelper.TABLE_AUDIOBOOKS, getContentValues(media), dbHelper.FIELD_ID + " = ?",
    ...
    public void deleteMedia(MediaItem media) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        db.delete(dbHelper.TABLE_AUDIOBOOKS, dbHelper.FIELD_ID + " = ?", new String[]{String.valueOf(media.getId())});
        db.close();
    }
    ...
    public void deleteAllMedias() {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        db.delete(dbHelper.TABLE_AUDIOBOOKS, null, null);
        db.close();
    }
