...
public abstract class BaseCategoryTableHelper extends BaseDbHelper {
    public BaseCategoryTableHelper(Context context) {
        super(context);
    }
    ...
    protected List<MediaItem> getSomeMedias(String tableName, String condition) {
        ...
        if (cursor.moveToFirst()) {
            do {
                MediaItem media = new MediaItem(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getInt(4) != 0, cursor.getInt(5));
                medias.add(media);
