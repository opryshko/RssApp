...
public class MoviesTableAdapterImpl extends BaseCategoryTableAdapter {
    ...
    public void addMediaItem(MediaItem media) {
        ...
        db.insert(dbHelper.TABLE_MOVIES, null, getContentValues(media));
        db.close();
    }
    ...
    public void addMediaItems(List<MediaItem> medias) {
        ...
        for (int i = 0; i < medias.size(); i++)
            db.insert(dbHelper.TABLE_MOVIES, null, getContentValues(medias.get(i)));
        db.close();
    }
    ...
    public MediaItem getMedia(int id) {
        ...
        Cursor cursor = db.query(dbHelper.TABLE_MOVIES, new String[]{dbHelper.FIELD_ID,
    ...
    public List<MediaItem> getAllMedias() {
        return getSomeMedias(dbHelper.TABLE_MOVIES, null);
    }
    ...
    public List<MediaItem> getFavouriteMedias() {
        ...
        return getSomeMedias(dbHelper.TABLE_MOVIES, dbHelper.FIELD_IS_FAVOURITE + "=1");
    }
    ...
    public int updateMedia(MediaItem media) {
        ...
        return db.update(dbHelper.TABLE_MOVIES, getContentValues(media), dbHelper.FIELD_ID + " = ?",
    ...
    public void deleteMedia(MediaItem media) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        db.delete(dbHelper.TABLE_MOVIES, dbHelper.FIELD_ID + " = ?", new String[]{String.valueOf(media.getId())});
        db.close();
    }
    ...
    public void deleteAllMedias() {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        db.delete(dbHelper.TABLE_MOVIES, null, null);
        db.close();
    }
